				  ...
				  struct safexcel_command_desc *cdesc)
{
	...
	if (ctx->mode == CONTEXT_CONTROL_CRYPTO_MODE_CTR_LOAD) {
		...
		return;
	} else if (ctx->mode == CONTEXT_CONTROL_CRYPTO_MODE_GCM) {
		cdesc->control_data.options |= EIP197_OPTION_4_TOKEN_IV_CMD;
				...
				u32 cryptlen, u32 assoclen, u32 digestsize)
{
	...
	if (direction == SAFEXCEL_ENCRYPT) {
		...
		token[5].opcode = EIP197_TOKEN_OPCODE_INSERT;
		...
		token[5].opcode = EIP197_TOKEN_OPCODE_RETRIEVE;
		token[5].packet_length = digestsize;
		token[5].stat = EIP197_TOKEN_STAT_LAST_HASH |
				EIP197_TOKEN_STAT_LAST_PACKET;
		token[5].instructions = EIP197_TOKEN_INS_INSERT_HASH_DIGEST;

		token[6].opcode = EIP197_TOKEN_OPCODE_VERIFY;
		token[6].packet_length = digestsize |
					 EIP197_TOKEN_HASH_RESULT_VERIFY;
		token[6].stat = EIP197_TOKEN_STAT_LAST_HASH |
				EIP197_TOKEN_STAT_LAST_PACKET;
		token[6].instructions = EIP197_TOKEN_INS_TYPE_OUTPUT;
	}
	...
	if (likely(cryptlen)) {
		...
		token[4].opcode = EIP197_TOKEN_OPCODE_DIRECTION;
		token[4].packet_length = cryptlen;
		token[4].stat = EIP197_TOKEN_STAT_LAST_HASH;
		token[4].instructions = EIP197_TOKEN_INS_LAST |
					EIP197_TOKEN_INS_TYPE_CRYPTO |
	...
	if (ctx->xcm) {
		...
		token[2].packet_length = 0;
		token[2].instructions = EIP197_TOKEN_INS_TYPE_OUTPUT;

		token[3].opcode = EIP197_TOKEN_OPCODE_INSERT;
		token[3].packet_length = AES_BLOCK_SIZE;
...
static int safexcel_aead_gcm_cra_init(struct crypto_tfm *tfm)
{
	...
	ctx->xcm = 1; /* GCM */
	ctx->mode = CONTEXT_CONTROL_CRYPTO_MODE_GCM; /* override default */
