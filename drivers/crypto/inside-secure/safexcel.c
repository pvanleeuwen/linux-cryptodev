...
static void safexcel_configure(struct safexcel_crypto_priv *priv)
{
	...
	priv->config.pes = priv->hwconfig.numpes;
	priv->config.rings = min_t(u32, priv->hwconfig.numrings, max_rings);
				  ...
				  int is_pci_dev)
{
	...
	if (priv->flags & SAFEXCEL_HW_EIP197) {
		...
		priv->hwconfig.numpes 	= (hiaopt >> EIP197_N_PES_OFFSET) &
					  EIP197_N_PES_MASK;
		priv->hwconfig.numrings = (hiaopt >> EIP197_N_RINGS_OFFSET) &
					  EIP197_N_RINGS_MASK;
	} else {
		/* EIP97 */
		priv->hwconfig.hwdataw  = (hiaopt >> EIP197_HWDATAW_OFFSET) &
		...
		priv->hwconfig.hwrfsize = (hiaopt >> EIP97_RFSIZE_OFFSET) &
					  EIP97_RFSIZE_MASK;
		priv->hwconfig.numpes 	= 1; /* by definition */
		priv->hwconfig.numrings = (hiaopt >> EIP197_N_RINGS_OFFSET) &
					  EIP197_N_RINGS_MASK;
}
	...
	dev_info(priv->dev, "EIP%d:%x(%d,%d,%d)-HIA:%x(%d,%d,%d),PE:%x,alg:%08x\n",
		 peid, priv->hwconfig.hwver, hwctg, priv->hwconfig.numpes,
		 priv->hwconfig.numrings, priv->hwconfig.hiaver,
